...
#
def tensor(w,v):
    """Funcion que le ayuda a calcular el tensor de una matriz o vector
       (list2d) -->list2d"""
    resp = [[(0,0) for x in range(len(w[0])*len(v[0]))]for x in range(len(w)*len(v))]
    for i in range(len(resp)):
        for j in range(len(resp[0])):
            resp[i][j] = multiplicar(w[i//len(v)][j//len(w)],v[i%len(v)][j%len(w)])
    return resp
#
